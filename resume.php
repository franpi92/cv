...
*/
$type_precedent='';
$requete = "SELECT * FROM `cv` WHERE `Type_paragraphe` NOT IN ('Type_experience', 'Titre_mission', 'Tache', 'Descriptif_experience', 'sous_tache') ORDER BY ID";
$i=0;
...
{   $texte = utf8_encode($row[4])."<br>";
    $date_debut = utf8_encode(strftime("%b %Y", strtotime($row[1])));
    $date_fin = utf8_encode(strftime("%b %Y", strtotime($row[2])));
    $type_paragraphe = utf8_encode($row[3]);
    $i++;
